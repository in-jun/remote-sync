...
class SyncAlertNotifier @Inject constructor(
    ...
    fun recordSuccess(pair: SyncPair) {
        prefs.edit { remove(failureKey(pair.id)) }
    ...
    fun forget(pairId: Long) {
        prefs.edit { remove(failureKey(pairId)) }
    ...
    private fun postAlert(id: Int, title: String, text: String) {
        // Checked inline so the permission requirement of notify() is visibly satisfied
        // here; on API 33+ posting without POST_NOTIFICATIONS is silently dropped.
    ...
    private fun countFailure(pairId: Long): Int {
        ...
        prefs.edit { putInt(failureKey(pairId), failures) }
        return failures
    }
