package dev.injun.remotesync.ui.settings
...
import dev.injun.remotesync.ui.openBatteryOptimizationSettings
            ...
            if (settings.mode == SyncMode.REALTIME && !rememberBatteryExemption()) {
                ...
                Text(
                    "Battery optimization is limiting real-time sync: when the device " +
                        ...
                        "periodic sync. In the list that opens, switch Remote Sync to " +
                        "\"Don't optimize\" to sync in the background.",
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.error,
                )
                TextButton(onClick = { openBatteryOptimizationSettings(context) }) {
