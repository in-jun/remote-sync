...
/**
 * Opens the system battery-optimization list so the user can exempt this app.
 *
 * The direct "ask for exemption" dialog needs REQUEST_IGNORE_BATTERY_OPTIMIZATIONS,
 * which is reserved for core use cases; the list is the sanctioned path for
 * everything else, so the user picks the app there.
 */
fun openBatteryOptimizationSettings(context: Context) {
    context.startFirstAvailable(
        Intent(Settings.ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS),
        // Fall back to the app details page, which links to battery usage.
        appDetailsIntent(context),
    )
}
