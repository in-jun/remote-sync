...
class SmbRemoteStorageIntegrationTest {
    ...
    fun setup() {
        ...
        config = SmbConfig(
            ...
            port = property("smb.port", "445").toInt(),
            shareName = property("smb.share", "sync"),
            username = property("smb.user", "smbuser"),
            password = property("smb.pass", "smbpass"),
            rootPath = "it-${System.nanoTime()}",
        )
    }

    private fun property(name: String, default: String): String = System.getProperty(name) ?: default
